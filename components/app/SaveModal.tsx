...
export function SaveModal({ isOpen, onClose, isLoggedIn, result, score, params, onSave }: SaveModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-md rounded-2xl border border-white/[0.1] bg-[#111113] shadow-2xl overflow-hidden">
        {/* Close */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 w-7 h-7 rounded-lg bg-white/[0.06] hover:bg-white/[0.1] flex items-center justify-center text-zinc-500 hover:text-white transition-colors"
        >
        ...
        {!isLoggedIn ? (
          ...
          <div className="p-8 text-center space-y-5">
            ...
            <div>
              <p className="text-lg font-bold text-white mb-2" style={{ letterSpacing: '-0.02em' }}>
            ...
            {result && score && (
              <div className="rounded-xl bg-white/[0.04] border border-white/[0.07] p-4 text-left space-y-2">
                <p className="text-xs text-zinc-500 font-medium">Simulation à sauvegarder</p>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-white font-semibold">{result.ville} · {formatCurrency(result.prixRevient)}</span>
          ...
          <div className="p-8 text-center space-y-5">
            ...
            <div>
              <p className="text-lg font-bold text-white mb-1">Simulation sauvegardée !</p>
              <p className="text-sm text-zinc-500">Elle est disponible dans votre dashboard.</p>
            </div>
            <div className="flex gap-2">
              <button
                onClick={onClose}
                className="flex-1 py-2 text-sm text-zinc-500 hover:text-white border border-white/[0.08] rounded-xl transition-colors"
              >
                Continuer l'analyse
              </button>
              <button
          ...
          <div className="p-8 space-y-5">
            <div>
              ...
              <p className="text-xs text-zinc-500">Donnez un nom à cette analyse pour la retrouver facilement.</p>
            </div>

            <div className="space-y-1.5">
              <label className="block text-xs font-medium text-zinc-400">Nom de la simulation</label>
              <input
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder={defaultName}
                className="w-full bg-white/[0.05] border border-white/[0.1] rounded-xl px-3 py-2.5 text-sm text-white placeholder:text-zinc-600 focus:outline-none focus:ring-1 focus:ring-emerald-500/50 focus:border-emerald-500/40 transition-all"
