...
const STYLES: Record<ToastType, { icon: string; bar: string; bg: string; border: string }> = {
  success: { icon: 'text-emerald-400', bar: 'bg-emerald-500', bg: 'bg-[#0d1f17]', border: 'border-emerald-500/25' },
  error:   { icon: 'text-red-400',     bar: 'bg-red-500',     bg: 'bg-[#1f0d0d]', border: 'border-red-500/25' },
  info:    { icon: 'text-blue-400',    bar: 'bg-blue-500',    bg: 'bg-[#0d1020]', border: 'border-blue-500/25' },
  warning: { icon: 'text-amber-400',   bar: 'bg-amber-500',   bg: 'bg-[#1f1a0d]', border: 'border-amber-500/25' },
}
...
function ToastItem({ toast, onRemove }: { toast: Toast; onRemove: (id: string) => void }) {
  ...
  return (
    ...
    >
      ...
      {/* Close */}
      <button className="shrink-0 text-zinc-600 hover:text-zinc-300 transition-colors ml-1">
