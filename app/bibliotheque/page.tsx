...
function ScoreRing({ score, size = 38 }: { score: number; size?: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ flexShrink: 0 }}>
      ...
      <text x={c} y={c} dy="0.355em" textAnchor="middle"
...
function CreateTagModal({ onClose, onCreate }: { onClose: () => void; onCreate: (tag: CustomTag) => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(0,0,0,.75)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={onClose}>
      <div style={{ background: 'var(--c-surface2)', border: '0.5px solid var(--c-border)', borderRadius: 14, padding: '24px 24px 20px', width: 340, boxShadow: '0 24px 64px rgba(0,0,0,.8)' }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--c-text-1)', letterSpacing: '-.02em' }}>Nouveau tag</p>
        ...
        <div style={{ marginBottom: 18 }}>
          <label style={{ display: 'block', fontSize: 11, color: 'var(--c-text-2)', marginBottom: 7, fontWeight: 500, letterSpacing: '.04em', textTransform: 'uppercase' }}>Nom du tag</label>
          <input autoFocus value={name} onChange={e => setName(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') handleCreate() }}
            ...
            style={{ width: '100%', background: 'var(--c-surface)', border: '0.5px solid var(--c-border)', borderRadius: 8, color: 'var(--c-text-1)', fontSize: 13, padding: '9px 12px', outline: 'none', boxSizing: 'border-box' }}
            onFocus={e => (e.target.style.borderColor = 'var(--c-border-med)')} onBlur={e => (e.target.style.borderColor = 'var(--c-border)')} />
        </div>
        <div style={{ marginBottom: 22 }}>
          <label style={{ display: 'block', fontSize: 11, color: 'var(--c-text-2)', marginBottom: 10, fontWeight: 500, letterSpacing: '.04em', textTransform: 'uppercase' }}>Couleur</label>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <p style={{ fontSize: 11, color: 'var(--c-text-3)' }}>Couleur libre</p>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={onClose} style={{ flex: 1, padding: 9, background: 'none', border: '0.5px solid var(--c-border)', borderRadius: 8, color: 'var(--c-text-2)', fontSize: 13, cursor: 'pointer' }}>Annuler</button>
    ...
    <div style={{ position: 'relative' }}>
      ...
      {open && mounted && createPortal(
        ...
        >
          ...
          <div style={{ borderTop: '0.5px solid var(--c-border)', margin: '4px 0', paddingTop: 4 }}>
            <button onClick={e => { e.stopPropagation(); setOpen(false); onCreateTag() }} style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '6px 10px', borderRadius: 6, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--c-text-2)', fontSize: 12 }}>
...
function SimCard({ sim, allTags, onUpdate, onDelete, onClick, onCreateTag }: { sim: SavedSimulation; allTags: TagDef[]; onUpdate: (id: string, tags: string[]) => void; onDelete: (id: string) => void; onClick: (id: string) => void; onCreateTag: () => void }) {
  ...
  return (
    <div
      style={{ background: 'var(--c-surface)', border: '1px solid var(--c-border)', borderRadius: 10, overflow: 'hidden', display: 'flex', flexDirection: 'column', cursor: 'pointer', transition: 'border-color .15s', boxShadow: 'var(--c-shadow-card)' }}
      onMouseEnter={e => (e.currentTarget.style.borderColor = 'var(--c-border-med)')}
    ...
    >
        ...
        <div style={{ padding: '11px 13px 10px' }} onClick={() => onClick(sim.id)}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 9 }}>
            <div style={{ flex: 1, minWidth: 0, marginRight: 10 }}>
              <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--c-text-1)', letterSpacing: '-.01em', marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{sim.name}</p>
              <p style={{ fontSize: 11, color: 'var(--c-text-3)' }}>{sim.ville} · {fmt(sim.prixAchat)} EUR</p>
            </div>
          ...
          <div style={{ display: 'flex', gap: 14, marginBottom: 10 }}>
            {[
              ...
              { label: 'Regime', value: (sim.params as Record<string,string>)?.regimeFiscal || 'LMNP', color: 'var(--c-text-2)' },
            ].map(k => (
              <div key={k.label}>
                <p style={{ fontSize: 9, color: 'var(--c-text-3)', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 2 }}>{k.label}</p>
          ...
          {/* Score breakdown */}
          <div style={{ borderTop: '0.5px solid var(--c-border)', paddingTop: 9, display: 'flex', flexDirection: 'column', gap: 5 }}>
            {[
      ...
      {/* Footer tags */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap', padding: '7px 12px', borderTop: '0.5px solid var(--c-border)' }}>
        {sim.tags.map(tid => {
        ...
        {sim.tags.length === 0 && <span style={{ fontSize: 10, color: 'var(--c-text-3)' }}>Aucun tag</span>}
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 4, position: 'relative' }}>
          ...
          <button onClick={e => { e.stopPropagation(); setMenuOpen(o => !o) }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--c-text-3)', padding: '2px 3px' }}>
          ...
          {menuOpen && (
            <div style={{ position: 'absolute', bottom: '100%', right: 0, zIndex: 50, background: 'var(--c-surface2)', border: '0.5px solid var(--c-border)', borderRadius: 8, padding: 4, marginBottom: 4, minWidth: 140, boxShadow: '0 8px 24px rgba(0,0,0,.6)' }} onMouseLeave={() => setMenuOpen(false)}>
              {[
...
function SimRow({ sim, allTags, onUpdate, onDelete, onClick, onCreateTag }: { sim: SavedSimulation; allTags: TagDef[]; onUpdate: (id: string, tags: string[]) => void; onDelete: (id: string) => void; onClick: (id: string) => void; onCreateTag: () => void }) {
  ...
  const col = scoreColor(score)
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '9px 14px', borderBottom: '0.5px solid var(--c-border)', cursor: 'pointer', transition: 'background .12s' }} onMouseEnter={e => (e.currentTarget.style.background = 'var(--c-surface2)')} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
      <ScoreRing score={score} size={30} />
      <div style={{ flex: 1, minWidth: 0 }} onClick={() => onClick(sim.id)}>
        <p style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--c-text-1)', marginBottom: 1, letterSpacing: '-.01em', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{sim.name}</p>
        <p style={{ fontSize: 11, color: 'var(--c-text-3)' }}>{sim.ville} · {fmt(sim.prixAchat)} EUR</p>
      </div>
      <div style={{ display: 'flex', gap: 20, flexShrink: 0 }}>
        <div style={{ textAlign: 'right' }}>
          <p style={{ fontSize: 9, color: 'var(--c-text-3)', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 1 }}>Rdt net</p>
        ...
        <div style={{ textAlign: 'right' }}>
          <p style={{ fontSize: 9, color: 'var(--c-text-3)', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 1 }}>Cashflow</p>
      ...
      <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
        ...
        {sim.tags.length > 3 && <span style={{ fontSize: 10, color: 'var(--c-text-3)' }}>+{sim.tags.length - 3}</span>}
      </div>
      <TagPicker sim={sim} allTags={allTags} onUpdate={onUpdate} onCreateTag={onCreateTag} />
    </div>
  )
...
function KanbanView({ sims, allTags, onUpdate, onDelete, onClick, onCreateTag }: { sims: SavedSimulation[]; allTags: TagDef[]; onUpdate: (id: string, tags: string[]) => void; onDelete: (id: string) => void; onClick: (id: string) => void; onCreateTag: () => void }) {
  ...
  const cols = [...tagCols, ...(untagged.length > 0 ? [{ id: '__none__', label: 'Non classe', color: 'var(--c-text-3)' } as TagDef] : [])]
      ...
      {cols.map(col => {
        ...
        return (
          <div key={col.id} style={{ minWidth: 240, maxWidth: 260, flexShrink: 0 }}>
            {/* Header colonne */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '6px 10px', marginBottom: 8, background: 'var(--c-surface)', borderRadius: 8, border: '0.5px solid var(--c-border)' }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: col.color, flexShrink: 0 }} />
              <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--c-text-2)', flex: 1 }}>{col.label}</span>
              <span style={{ fontSize: 10, color: 'var(--c-text-3)', background: 'var(--c-surface3)', borderRadius: 10, padding: '1px 6px' }}>{colSims.length}</span>
            </div>
            {/* Cards */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
              {colSims.map(sim => {
                ...
                return (
                  <div key={sim.id} onClick={() => onClick(sim.id)} style={{ background: 'var(--c-surface)', border: '0.5px solid var(--c-border)', borderRadius: 9, overflow: 'hidden', cursor: 'pointer', transition: 'border-color .12s' }} onMouseEnter={e => (e.currentTarget.style.borderColor = 'var(--c-border-med)')} onMouseLeave={e => (e.currentTarget.style.borderColor = 'var(--c-border)')}>
                    <div style={{ height: 2.5, background: c }} />
                    <div style={{ padding: '9px 11px' }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 7 }}>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{ fontSize: 12, fontWeight: 600, color: 'var(--c-text-1)', marginBottom: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{sim.name}</p>
                          <p style={{ fontSize: 10, color: 'var(--c-text-3)' }}>{sim.ville} · {fmt(sim.prixAchat)} EUR</p>
                        </div>
                        <ScoreRing score={score} size={30} />
                      </div>
                      <div style={{ display: 'flex', gap: 12 }}>
                        <div>
                          <p style={{ fontSize: 9, color: 'var(--c-text-3)', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 1 }}>Rdt</p>
                        ...
                        <div>
                          <p style={{ fontSize: 9, color: 'var(--c-text-3)', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: 1 }}>CF</p>
              ...
              {colSims.length === 0 && (
                <div style={{ textAlign: 'center', padding: '24px 0', fontSize: 11, color: 'var(--c-text-3)' }}>Aucun bien</div>
              )}
            </div>
          </div>
        )
  ...
  return (
    <>
      ...
      <AppShell activeTag={tagFilter} onTagFilter={setTagFilter} customTags={customTags} onCreateTag={() => setShowCreateModal(true)}>
        <div style={{ minHeight: '100vh', background: 'var(--c-bg)', color: 'var(--c-text-1)', display: 'flex', flexDirection: 'column' }}>

          {/* Topbar */}
          <div style={{ padding: '14px 24px 12px', borderBottom: '0.5px solid var(--c-border)', display: 'flex', alignItems: 'center', gap: 10 }}>
            <h1 style={{ fontSize: 15, fontWeight: 600, letterSpacing: '-.02em', color: 'var(--c-text-1)', flex: 1 }}>
              Bibliotheque
              <span style={{ marginLeft: 8, fontSize: 11, fontWeight: 500, color: 'var(--c-text-3)' }}>{tagFilter === 'all' ? simulations.length : filtered.length} bien{simulations.length !== 1 ? 's' : ''}</span>
            </h1>
            {/* Search */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, background: 'var(--c-surface)', border: '0.5px solid var(--c-border)', borderRadius: 8, padding: '6px 12px', width: 200 }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--c-text-3)" strokeWidth={2}><circle cx="11" cy="11" r="8" /><path strokeLinecap="round" d="m21 21-4.35-4.35" /></svg>
              <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher..." style={{ background: 'none', border: 'none', outline: 'none', fontSize: 12, color: 'var(--c-text-2)', width: '100%' }} />
            </div>
            {/* Sort */}
            <select value={sortKey} onChange={e => setSortKey(e.target.value as SortKey)} style={{ background: 'var(--c-surface)', border: '0.5px solid var(--c-border)', borderRadius: 8, color: 'var(--c-text-2)', fontSize: 12, padding: '6px 10px', cursor: 'pointer', outline: 'none' }}>
              {SORT_OPTIONS.map(o => <option key={o.key} value={o.key}>{o.label}</option>)}
            </select>
            {/* Vue toggle */}
            <div style={{ display: 'flex', border: '0.5px solid var(--c-border)', borderRadius: 8, overflow: 'hidden' }}>
              {viewModes.map((v, i) => (
          ...
          <div style={{ flex: 1, padding: '16px 24px' }}>
            {loading ? (
              <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--c-text-3)', fontSize: 13 }}>Chargement...</div>
            ) : filtered.length === 0 ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '80px 0', gap: 12 }}>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: 'var(--c-surface)', border: '0.5px solid var(--c-border)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <p style={{ fontSize: 14, fontWeight: 500, color: 'var(--c-text-2)' }}>{search ? 'Aucun resultat' : tagFilter !== 'all' ? 'Aucun bien avec ce tag' : 'Aucune simulation sauvegardee'}</p>
                {!search && tagFilter === 'all' && (
                  <button onClick={() => router.push('/analyse')} style={{ marginTop: 4, padding: '8px 16px', background: 'var(--c-accent)', color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>Analyser un bien</button>
                )}
              </div>
            ) : (
              <>
                ...
                {viewMode === 'list' && (
                  <div style={{ background: 'var(--c-surface)', border: '0.5px solid var(--c-border)', borderRadius: 10, overflow: 'hidden' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 14px', borderBottom: '0.5px solid var(--c-border)' }}>
                      <span style={{ width: 30, flexShrink: 0, fontSize: 10, color: 'var(--c-text-3)', textTransform: 'uppercase', letterSpacing: '.06em' }}>Score</span>
                      <span style={{ flex: 1, fontSize: 10, color: 'var(--c-text-3)', textTransform: 'uppercase', letterSpacing: '.06em' }}>Bien</span>
                      <span style={{ width: 100, fontSize: 10, color: 'var(--c-text-3)', textTransform: 'uppercase', letterSpacing: '.06em', textAlign: 'right' }}>Rdt net</span>
                      <span style={{ width: 100, fontSize: 10, color: 'var(--c-text-3)', textTransform: 'uppercase', letterSpacing: '.06em', textAlign: 'right' }}>Cashflow</span>
                      <span style={{ width: 60, fontSize: 10, color: 'var(--c-text-3)', textTransform: 'uppercase', letterSpacing: '.06em' }}>Tags</span>
                      <span style={{ width: 30 }} />
                    </div>
