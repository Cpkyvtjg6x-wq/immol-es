      ...
      <aside className={`fixed inset-y-0 left-0 w-56 flex flex-col z-40 transition-transform duration-200 ${
          ...
          <div className="px-3 py-2.5 shrink-0" style={{ borderBottom: '1px solid var(--c-sidebar-border)' }}>
            {isPro ? (
              ...
              >
                ...
                <span className="text-[11px] font-semibold text-zinc-400 group-hover:text-zinc-300">Gratuit</span>
        ...
        <nav className="flex-1 overflow-y-auto p-2 pt-3">
              ...
              return (
                ...
                >
                  ...
                  {'badge' in item && item.badge && (
                    <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-md bg-white/[0.06] text-zinc-400 border border-white/[0.07] tabular-nums">
                      {item.badge}
                    </span>
                  )}
                </Link>
          ...
          {isBibliotheque && onTagFilter && (
            <div className="mb-5">
              <p className="px-3 mb-1.5 text-[10px] font-semibold text-zinc-400 uppercase tracking-widest">Mes tags</p>
              <div className="space-y-0">
                ...
                >
                  ...
                  <span className="text-[10px] text-zinc-400 tabular-nums mr-1">{simulations.length}</span>
                  {/* Chevron rotatif */}
                  <svg
                    className="w-3 h-3 shrink-0 text-zinc-400 transition-transform duration-250"
                ...
                >
                  <div className="pt-0.5 space-y-0">
                    ...
                    {allSidebarTags.map(t => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {count > 0 && <span className="text-[10px] text-zinc-400 tabular-nums shrink-0">{count}</span>}
                        </button>
                      )
                    })}
                    ...
                    {onCreateTag && (
                      <button
                        onClick={onCreateTag}
                        className="w-full flex items-center gap-2.5 px-3 py-[6px] rounded-lg text-[12.5px] transition-all text-left text-zinc-400 hover:text-zinc-400 hover:bg-white/[0.03]"
                      >
          ...
          {recentSims.length > 0 && !isBibliotheque && (
            <div className="mb-5">
              <p className="px-3 mb-2 text-[10px] font-semibold text-zinc-400 uppercase tracking-widest">Recentes</p>
              ...
              <Link
                href="/dashboard"
                className="flex items-center gap-1.5 px-3 py-1.5 mt-1 text-[10px] font-semibold text-zinc-400 hover:text-zinc-400 transition-colors"
              >
          ...
          {/* Outils Pro */}
          <div>
            <p className="px-3 mb-2 text-[10px] font-semibold text-zinc-400 uppercase tracking-widest">Outils Pro</p>
            <div className="space-y-0.5">
              {tools.map((item) => {
                ...
                if (item.disabled) {
                  return (
                    <button key={item.label} disabled className={cls}>
                      ...
                      {item.badge && (
                        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-md bg-white/[0.05] text-zinc-400 border border-white/[0.07]">
                          {item.badge}
                        </span>
                      )}
                    </button>
          ...
          {user ? (
            <>
              <div className="flex items-center gap-1 px-1 pb-1">
                <NotificationBell simulationCount={simulations.length} isPro={isPro} />
                <p className="text-[10px] text-zinc-400 ml-1">Notifications</p>
              </div>
              <Link
                href="/profile"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-zinc-400 leading-tight truncate group-hover:text-zinc-300 transition-colors">Mon profil</p>
                </div>
                <svg className="w-3 h-3 text-zinc-400 group-hover:text-zinc-400 shrink-0 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
                </svg>
              </Link>
              <button
                onClick={signOut}
                className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-[13px] text-zinc-400 hover:text-zinc-300 hover:bg-white/[0.04] transition-all"
              >
