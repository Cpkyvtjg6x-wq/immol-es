...
export function MarketContextBlock({ data, surface, prixAchat, loading }: Props) {
  ...
  if (loading) {
    return (
      <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] px-6 py-8 flex items-center gap-3">
        <div className="w-4 h-4 border-[1.5px] border-zinc-700 border-t-emerald-400 rounded-full animate-spin shrink-0" />
        <span className="text-[12px] text-zinc-600">Analyse du marché local en cours…</span>
      </div>
    )
  }
  ...
  const positionColor: Record<string, string> = {
    ...
    'dans-marche': 'text-zinc-300',
    'sur-marche':  'text-amber-400',
    'inconnu':     'text-zinc-600',
  }
  ...
  return (
    <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider">
              Marché local
              {data.quartier && <span className="text-zinc-600"> · {data.quartier}</span>}
            </p>
            {data.adresse && (
              <p className="text-[10px] text-zinc-600 truncate mt-0.5">{data.adresse}</p>
            )}
          </div>
        </div>
        <div className="flex items-center gap-1.5 shrink-0">
          <span className={`w-1.5 h-1.5 rounded-full ${sourceDot}`} />
          <span className="text-[10px] text-zinc-700">{sourceLabel}</span>
        </div>
      </div>
      ...
      <div className="px-5 py-4 grid grid-cols-2 gap-6 border-b border-white/[0.05]">
        ...
        <div className="space-y-2.5">
          ...
          {prixM2Achat > 0 ? (
            ...
            <p className="text-[12px] text-zinc-600 italic">Renseignez le prix d'achat</p>
          )}
        </div>
        ...
        <div className="space-y-2.5">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[15px] font-black text-white" style={{ letterSpacing: '-0.03em' }}>
      ...
      <div className="px-5 py-4 flex items-start gap-4 flex-wrap">
        ...
        {data.prixM2Ville > 0 && data.prixM2Median !== data.prixM2Ville && (
          <div className="flex-1 min-w-[180px] flex items-start gap-3 p-3 rounded-xl bg-white/[0.03] border border-white/[0.06]">
            ...
            <div>
              ...
              <p className="text-[15px] font-black text-zinc-300 tabular-nums" style={{ letterSpacing: '-0.02em' }}>
                {data.prixM2Ville.toLocaleString('fr-FR')} €/m²
              </p>
              {(() => {
                const diff = Math.round(((data.prixM2Median - data.prixM2Ville) / data.prixM2Ville) * 100)
                const color = Math.abs(diff) < 5 ? 'text-zinc-600' : diff > 0 ? 'text-amber-400' : 'text-emerald-400'
                return (
